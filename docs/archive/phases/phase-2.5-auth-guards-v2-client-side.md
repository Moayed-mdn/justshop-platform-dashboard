> ℹ️ **Final approach for Phase 2.5.** Supersedes the server-side attempt in
> [`phase-2.5-auth-guards-v1-server-side.md`](./phase-2.5-auth-guards-v1-server-side.md).
> Auth checking moved to the client (`useEffect` + `/api/auth/me`) instead of the dashboard
> layout server component. Note the doc's own "Known Limitations" section: there is a brief
> flash of protected content before the client-side redirect fires — worth re-checking against
> current code if server-side protection matters for this app.
